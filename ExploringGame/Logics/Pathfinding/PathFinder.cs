...
public class PathFinder
{
    ...
    private Vector3 ComputeTargetDirection(GameTime gameTime)
    {
        ...
        {
            CurrentTarget = PrimaryTarget;
            return Vector3.Normalize(PrimaryTarget.Target.WorldPosition - _entity.WorldPosition);
        }
        ...
        if (HasReachedTarget())
        {
            ...
            return Vector3.Normalize(CurrentTarget.Target.WorldPosition - _entity.WorldPosition);
        }
        ...
        if (IsStuckOnObstacle())
        {
            ...
            {
                _samplePoint.WorldPosition = obstacleAvoidanceSamplePoint.Value;
        ...
        {
            _samplePoint.WorldPosition = samplePoint.Value;
            CurrentTarget = new PathFinderTarget(_samplePoint);
            return Vector3.Normalize(CurrentTarget.Target.WorldPosition - _entity.WorldPosition);
        }
        ...
        var directionToTarget = Vector3.Normalize(CurrentTarget.Target.WorldPosition - _entity.WorldPosition);
    ...
    {
        var directionToTarget = CurrentTarget.Target.WorldPosition - _entity.WorldPosition;
    ...
    {
        var startWaypoint = _waypointGraph.FindNearestWaypoint(_entity.WorldPosition);
        var goalWaypoint = _waypointGraph.FindNearestWaypoint(PrimaryTarget.Target.WorldPosition);
    ...
    private bool IsStuckOnObstacle()
    {
        ...
        var currentWaypoint = _waypointGraph.FindNearestWaypoint(_entity.WorldPosition);
        if (currentWaypoint == null)
            return false;

        var currentWaypointDistanceToTarget = Vector3.Distance(currentWaypoint.LocalPosition, PrimaryTarget.Target.WorldPosition);
        ...
        foreach (var waypoint in _waypointGraph.GetAllWaypoints())
        {
            ...
            var distanceToTarget = Vector3.Distance(waypoint.WorldPosition, PrimaryTarget.Target.WorldPosition);
